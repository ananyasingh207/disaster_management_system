  ...
  return (
    <div className="max-w-6xl mx-auto pb-12 animate-fade-in-up">
      ...
      <div className="mb-10 border-b border-slate-800 pb-6">
        ...
        <p className="text-slate-400 font-medium">Official broadcasts and emergency warnings for your region.</p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6">
        ...
        {alerts.length === 0 && (
          <div className="text-center py-16 border-2 border-dashed border-slate-800 rounded-2xl bg-slate-900/50">
            ...
            <p className="text-slate-500 text-sm">There are currently no active alerts for your area.</p>
          </div>
        )}
        ...
        {alerts.map((a) => {
          ...
          const icon = a.type === 'WEATHER' ? '⛈️' : a.type === 'EVACUATION' ? '📢' : '⚠️';

          return (
            <div key={a._id} className={`relative bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl overflow-hidden group hover:border-slate-700 transition-colors`}>
                ...
                <div className="flex-1 pl-4">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className="text-[10px] font-mono text-slate-500 flex items-center gap-1">
                      <span>🕒</span> {new Date(a.createdAt).toLocaleString()}
                    </span>
                  </div>
